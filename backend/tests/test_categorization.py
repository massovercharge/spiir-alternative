...
def seeded_db(engine, _patch_engine):
    ...
    with Session(engine) as db:
        ...
        acc = Account(
            uid="test-acc", household_id=TEST_HOUSEHOLD_ID, name="Test Account", source="test"
        )
        db.add(acc)
        ...
        p1 = Posting(
            ...
            account_uid="test-acc",
            booking_date="2026-07-01",
            amount_minor=-5000,
            original_description="Kaffebaren aps",
            currency="DKK",
        )
        p2 = Posting(
            id="p2",
        ...
        db.add(
            PostingAllocation(
                posting_id="p1", amount_minor=-5000, category_id="diverse|ikke-kategoriseret"
            )
        )
        db.add(
            PostingAllocation(
                posting_id="p2", amount_minor=-3000, category_id="diverse|ikke-kategoriseret"
            )
        )
        db.add(
            PostingAllocation(
                posting_id="p3", amount_minor=-20000, category_id="dagligvarer|supermarked"
            )
        )
...
def test_update_transaction_category(seeded_db):
    ...
    with Session(seeded_db) as db:
        ...
        alloc2 = db.exec(
            select(PostingAllocation).where(PostingAllocation.posting_id == "p2")
        ).first()
