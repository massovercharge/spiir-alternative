...
def seeded_db(engine, _patch_engine):
    ...
    with Session(engine) as db:
        ...
        db.add(Posting(id="p1", household_id=TEST_HOUSEHOLD_ID, account_uid="acc1", amount_minor=-9900, booking_date="2026-05-01", original_description="NETFLIX.COM"))
        db.add(Posting(id="p2", household_id=TEST_HOUSEHOLD_ID, account_uid="acc1", amount_minor=-9900, booking_date="2026-06-01", original_description="NETFLIX.COM"))
        db.add(Posting(id="p3", household_id=TEST_HOUSEHOLD_ID, account_uid="acc1", amount_minor=-9900, booking_date="2026-07-01", original_description="NETFLIX.COM"))

        # 2 Spotify payments (not enough for recurring detection which requires 3)
        db.add(Posting(id="p4", household_id=TEST_HOUSEHOLD_ID, account_uid="acc1", amount_minor=-10900, booking_date="2026-06-05", original_description="Spotify AB"))
        db.add(Posting(id="p5", household_id=TEST_HOUSEHOLD_ID, account_uid="acc1", amount_minor=-10900, booking_date="2026-07-05", original_description="Spotify AB"))
...
class TestRecurringMatching:
    def test_match_and_advance_date(self, seeded_db):
        ...
        posting = Posting(id="p_new", household_id=TEST_HOUSEHOLD_ID, account_uid="acc1", amount_minor=-9900, booking_date="2026-07-01T10:00:00Z", original_description="VISA KØB NETFLIX.COM")
