...
def seeded_session_fixture(engine):
    ...
    with Session(engine) as session:
        ...
        # Create categories
        session.add(
            Category(id="husholdning|dagligvarer", main_name="Husholdning", sub_name="Dagligvarer")
        )
        session.add(
            Category(id="transport|braendstof", main_name="Transport", sub_name="Brændstof")
        )
        session.add(
            Category(
                id="indkomst|loen", main_name="Indkomst", sub_name="Løn", category_type="Income"
            )
        )
        session.add(
            Category(
                id="diverse|ikke-kategoriseret", main_name="Diverse", sub_name="Ikke kategoriseret"
            )
        )
        ...
        session.add(
            Posting(
                ...
                amount_minor=-45000,
                currency="DKK",
                original_description="Circle K benzin",
            )
        )
        session.add(
            Posting(
...
class TestPostingAllocation:
    ...
    def test_split_allocations_sum(self, session: Session):
        ...
        session.add(
            PostingAllocation(posting_id="p1", category_id="food|groceries", amount_minor=-7000)
        )
        session.add(
            PostingAllocation(posting_id="p1", category_id="household|cleaning", amount_minor=-3000)
        )
        session.commit()
...
class TestBankConnection:
    def test_create(self, session: Session):
        conn = BankConnection(
            household_id=TEST_HOUSEHOLD_ID, provider="enablebanking", bank_name="Nordea"
        )
        session.add(conn)
        session.commit()
        ...
        try:
            ...
            for tx in result["transactions"]:
                assert isinstance(tx["amount"], str), (
                    f"amount should be str, got {type(tx['amount'])}"
                )
