...
function AccountHistoryChart({ uid }: { uid: string }) {
  ...
  if (!history || history.length === 0)
    return <div className="p-4 text-center text-muted">Ingen historik fundet.</div>;
  ...
  return (
    <div className="h-48 w-full mt-6 pt-6 border-t border-[hsl(var(--border-color))]">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart
          data={history}
          margin={{ top: 5, right: 5, left: isMobile ? -10 : 10, bottom: 5 }}
        >
          ...
          <Tooltip
            formatter={(value: any) => [
              (value / 100).toLocaleString('da-DK', { style: 'currency', currency: 'DKK' }),
              'Saldo',
            ]}
            labelFormatter={(label) =>
              new Date(label).toLocaleDateString('da-DK', { dateStyle: 'long' })
            }
            contentStyle={{
            ...
            return (
              <div key={group} className="space-y-4">
                <div className="flex items-center justify-between px-2">
                  <h2 className="text-xl font-semibold text-[hsl(var(--text-secondary))]">
                    {group}
                  </h2>
                  <span
                    className={`font-semibold ${groupTotal < 0 ? 'text-[hsl(var(--brand-danger))]' : 'text-[hsl(var(--text-primary))]'}`}
                  >
                    {(groupTotal / 100).toLocaleString('da-DK', {
                      style: 'currency',
                      currency: 'DKK',
                    })}
                  </span>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex-1">
                              {editingId === acc.uid ? (
                                <div className="flex flex-col gap-2 w-full md:w-auto">
                                  ...
                                  >
                                    ...
                                    <div className="flex gap-1 mt-2 md:mt-0">
                                      <Button
                                        size="sm"
                                        onClick={(e) => handleSave(acc.uid, e as any)}
                                      >
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <button
                                    ...
                                    className="p-1 rounded-md text-muted hover:text-[hsl(var(--brand-primary))] hover:bg-[hsla(var(--brand-primary),0.1)] opacity-0 group-hover:opacity-100 transition-all"
                                  >
                                    <Pencil size={14} />
                                  </button>
                                </div>
                              ...
                              <p className="text-sm text-muted mt-1">
                                {acc.bank_connection?.bank_name || t('accounts.unknown_bank')}{' '}
                                &middot; {acc.currency || 'DKK'}
                              </p>
                            </div>
                            ...
                            <div className="flex items-center gap-4 text-right">
                              <div>
                                <p
                                  className={`font-bold text-xl ${acc.balance_minor < 0 ? 'text-[hsl(var(--brand-danger))]' : 'text-[hsl(var(--text-primary))]'}`}
                                >
                                  {acc.balance_minor
                                    ? (acc.balance_minor / 100).toLocaleString('da-DK', {
                                        style: 'currency',
                                        currency: 'DKK',
                                      })
                                    : '0,00 kr.'}
