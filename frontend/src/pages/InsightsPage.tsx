...
import { useInsightsSunburst, useIncomeExpenseSeries, useCategoryDrilldown, useInsightsAverages } from '../api/client';
...
function CategoryDrilldownView({ category, year, onClose, isMobile }: { category: string, year: number, onClose: () => void, isMobile: boolean }) {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-[hsl(var(--bg-secondary))] p-4 border border-[hsl(var(--border-color))] rounded-lg shadow-xl w-72 z-50">
          <p className="font-medium mb-2 text-[hsl(var(--text-primary))]">{new Date(label).toLocaleDateString('da-DK', { month: 'long', year: 'numeric' })}</p>
          ...
          {monthData && monthData.top_transactions.length > 0 && (
            <div className="space-y-2 mt-2 pt-3 border-t border-[hsl(var(--border-color))]">
              <p className="text-xs text-muted font-semibold uppercase tracking-wider mb-2">Største poster</p>
              {monthData.top_transactions.map((tx: any, idx: number) => (
                <div key={idx} className="flex justify-between items-center text-sm">
                  <span className="truncate w-2/3 pr-2" title={tx.payee}>{idx + 1}. {tx.payee}</span>
                  <span className="font-medium w-1/3 text-right">{(Math.abs(tx.amount_minor) / 100).toLocaleString('da-DK')}</span>
                </div>
              ))}
            </div>
          )}
  ...
  const formattedData = data?.monthly_data.map((m: any) => ({
    name: m.month,
      ...
      <Card className="h-full border-[hsl(var(--brand-primary))] border-2 shadow-lg">
        ...
        <CardContent className="h-[340px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={formattedData} margin={{ top: 20, right: 30, left: isMobile ? -15 : 10, bottom: 5 }}>
              ...
              />
              <RechartsTooltip content={<DrilldownTooltip />} cursor={{ stroke: 'hsl(var(--border-color))', strokeWidth: 1, strokeDasharray: '5 5' }} />
              <Line 
                type="monotone" 
                dataKey="value" 
                stroke="hsl(var(--brand-primary))" 
                strokeWidth={4} 
                dot={{ r: 4, fill: "hsl(var(--brand-primary))", strokeWidth: 2, stroke: "hsl(var(--bg-primary))" }} 
...
export default function InsightsPage() {
  ...
  const { data: sunburstData, isLoading: isLoadingSunburst } = useInsightsSunburst({ year, filterType: sunburstFilterType });
  ...
  const LineTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-[hsl(var(--bg-secondary))] p-3 border border-[hsl(var(--border-color))] rounded-lg shadow-xl pointer-events-none z-50 relative">
          <p className="font-medium mb-2">{new Date(label).toLocaleDateString('da-DK', { month: 'long', year: 'numeric' })}</p>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3 space-y-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-2xl font-normal text-[hsl(var(--text-secondary))]">Indkomst og udgifter</CardTitle>
              </CardHeader>
              <CardContent className="h-[400px]">
                {isLoadingTrend ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={formattedTrendData} margin={{ top: 20, right: 30, left: isMobile ? -15 : 20, bottom: 5 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="hsl(var(--border-color))" />
                      <XAxis 
                        dataKey="name" 
                        stroke="hsl(var(--text-secondary))" 
                        ...
                        tickFormatter={(val) => new Date(val).toLocaleDateString('da-DK', { month: 'short' })}
                      />
                      <YAxis stroke="hsl(var(--text-secondary))" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(value) => `${value}`} width={isMobile ? 45 : 60} />
                      <RechartsTooltip content={<LineTooltip />} />
                      
                      {/* Income bars going up */}
                      <Bar dataKey="Indkomst" fill="hsl(var(--brand-success))" radius={[4, 4, 0, 0]} maxBarSize={40} />
                      {/* Expense bars going down (NegativeUdgifter) */}
                      <Bar dataKey="NegativeUdgifter" name="Udgifter" fill="hsl(var(--brand-warning))" radius={[0, 0, 4, 4]} maxBarSize={40} />
                      
                      <Line type="monotone" dataKey="Resultat" stroke="hsl(var(--text-primary))" strokeWidth={3} dot={{ r: 4, fill: "hsl(var(--bg-primary))", strokeWidth: 2 }} activeDot={{ r: 6 }} />
                    </ComposedChart>
                  </ResponsiveContainer>
                )}
              </CardContent>
            ...
            <div className="h-[480px]">
              <AnimatePresence mode="wait">
                {selectedCategory ? (
                  ...
                  <motion.div key="topexpenses" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="h-full">
                    <Card className="h-full border-none shadow-none bg-transparent">
                      <CardHeader className="px-0">
                        <CardTitle className="text-xl font-normal text-[hsl(var(--text-secondary))]">Hvad har jeg brugt mine penge på?</CardTitle>
                      </CardHeader>
                      <CardContent className="h-[400px] px-0">
                        {isLoadingSunburst ? (
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={topCategories} layout="vertical" margin={{ top: 5, right: 30, left: 5, bottom: 5 }}>
                              <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="hsl(var(--border-color))" />
                              <XAxis type="number" hide />
                              <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fill: 'hsl(var(--text-primary))', fontSize: 12 }} width={isMobile ? 80 : 160} />
                              <RechartsTooltip content={<BarTooltip />} cursor={{ fill: 'hsl(var(--bg-tertiary))' }} />
                              <Bar 
                                dataKey="value" 
                                radius={[0, 4, 4, 0]} 
                                maxBarSize={30}
          ...
          <div className="lg:col-span-1">
            <div className="bg-[hsl(var(--bg-secondary))] p-6 rounded-xl border border-[hsl(var(--border-color))] sticky top-6 space-y-8">
              <div className="text-center">
                <p className="text-lg text-[hsl(var(--text-secondary))] font-medium mb-4">Sidste 12 mdr.</p>
                <p className="text-xs font-semibold text-muted tracking-wider uppercase mb-2">Resultat</p>
                <h2 className="text-3xl font-bold text-[hsl(var(--text-primary))]">
                  {isLoadingAvg ? <Skeleton className="h-10 w-32 mx-auto" /> : `${parseFloat(averages?.net_avg || "0").toLocaleString('da-DK')} kr`}
                </h2>
              ...
              <div className="space-y-3 pt-6 border-t border-[hsl(var(--border-color))]">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{parseFloat(incomeTotal).toLocaleString('da-DK')} kr</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[hsl(var(--text-secondary))]">Regninger</span>
                  <span className="font-medium text-[hsl(var(--text-primary))]">-{parseFloat(regningerTotal).toLocaleString('da-DK')} kr</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[hsl(var(--text-secondary))]">Forbrug</span>
                  <span className="font-medium text-[hsl(var(--text-primary))]">-{parseFloat(forbrugTotal).toLocaleString('da-DK')} kr</span>
                </div>
                <div className="flex justify-between font-semibold pt-3 border-t border-[hsl(var(--border-color))]">
                  ...
                  <span className="text-[hsl(var(--brand-success))]">{netTotal.toLocaleString('da-DK')} kr</span>
                </div>
              </div>

              <div className="pt-6">
                <div className="bg-[hsl(var(--bg-primary))] rounded-lg p-4 border border-[hsl(var(--border-color))] text-center shadow-sm">
                  <p className="text-xs font-semibold text-muted tracking-wider uppercase mb-1">Opsparing</p>
                  <p className="text-lg font-bold text-[hsl(var(--text-primary))]">{savingsTotal.toLocaleString('da-DK')} kr</p>
                </div>
                <div className="mt-4 text-center">
                  <p className="text-[10px] font-semibold text-muted tracking-wider uppercase mb-1">Resultat m. opsparing</p>
                  <p className="font-medium text-[hsl(var(--text-primary))]">{netWithSavings.toLocaleString('da-DK')} kr</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <Card className="h-[480px]">
            ...
            <CardContent className="h-[400px]">
              {isLoadingTrend ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={formattedTrendData} margin={{ top: 20, right: 30, left: isMobile ? -15 : 20, bottom: 5 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="hsl(var(--border-color))" />
                    <XAxis dataKey="name" stroke="hsl(var(--text-secondary))" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(val) => new Date(val).toLocaleDateString('da-DK', { month: 'short' })} />
                    <YAxis stroke="hsl(var(--text-secondary))" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(value) => `${value}`} width={isMobile ? 45 : 60} />
                    <RechartsTooltip content={<LineTooltip />} />
                    <Line type="monotone" dataKey={dataKey} stroke={lineColor} strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
                  </LineChart>
                </ResponsiveContainer>
              )}
            </CardContent>
                  ...
                  <Card className="h-full">
                    ...
                    <CardContent className="h-[400px]">
                      {isLoadingSunburst ? (
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={topCategories} layout="vertical" margin={{ top: 5, right: 30, left: isMobile ? 5 : 20, bottom: 5 }}>
                            <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} stroke="hsl(var(--border-color))" />
                            <XAxis type="number" hide />
                            <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fill: 'hsl(var(--text-primary))', fontSize: 12 }} width={isMobile ? 80 : 120} />
                            <RechartsTooltip content={<BarTooltip />} cursor={{ fill: 'hsl(var(--bg-tertiary))' }} />
                            <Bar 
                              dataKey="value" 
                              radius={[0, 4, 4, 0]} 
              ...
              {isLoadingTrend ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={formattedTrendData} margin={{ top: 20, right: 30, left: isMobile ? -15 : 20, bottom: 5 }}>
                    ...
                    <YAxis stroke="hsl(var(--text-secondary))" fontSize={12} tickLine={false} axisLine={false} tickFormatter={(value) => `${value}`} width={isMobile ? 45 : 60} />
                    <RechartsTooltip content={<LineTooltip />} />
                    <Line type="monotone" dataKey="Indkomst" stroke="hsl(var(--brand-success))" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
                  </LineChart>
                </ResponsiveContainer>
              )}
            </CardContent>
          ...
          <div className="h-[480px]">
            <AnimatePresence mode="wait">
              {selectedCategory ? (
                ...
                <motion.div key="topincome" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="h-full">
                  <Card className="h-full">
                    <CardHeader>
    ...
    >
      <div className="mb-6 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
          <PeriodSelector mode="year" date={selectedDate} onChange={(d) => { setSelectedDate(d); setSelectedCategory(null); }} />
        </motion.div>
      </div>
