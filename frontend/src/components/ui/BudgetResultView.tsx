...
export function BudgetResultView({ summary, currentYear, currentMonth, isCurrentYear }: any) {
  ...
  const chartData = useMemo(() => {
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      let monthBudgetIncome = 0;
      let monthActualIncome = 0;
      let monthBudgetExpense = 0;
      let monthActualExpense = 0;

      budgetedIncome.forEach((cat: any) => {
        const mData = cat.months.find((x: any) => x.month === m);
        if (mData) {
          monthBudgetIncome += Math.abs(mData.budgeted_minor);
          monthActualIncome += Math.abs(mData.actual_minor);
        }
      });

      budgetedExpenses.forEach((cat: any) => {
        const mData = cat.months.find((x: any) => x.month === m);
        if (mData) {
          monthBudgetExpense += Math.abs(mData.budgeted_minor);
          monthActualExpense += Math.abs(mData.actual_minor);
        }
      });

      const monthBudgetNet = monthBudgetIncome - monthBudgetExpense;
      const monthActualNet = monthActualIncome - monthActualExpense;
    ...
    return data;
  }, [budgetedIncome, budgetedExpenses, currentMonth, isCurrentYear]);

  // Calculate year totals
  const totalIncome = budgetedIncome.reduce((sum: number, c: any) => sum + Math.abs(c.total_actual_minor), 0) / 100;
  ...
  const totalBills = bills.reduce((sum: number, c: any) => sum + Math.abs(c.total_actual_minor), 0) / 100;
  ...
  const totalConsumption = consumption.reduce((sum: number, c: any) => sum + Math.abs(c.total_actual_minor), 0) / 100;
  ...
  // Final predicted balance (Budget)
  const projectedBalance = totalIncomeBudget - (totalBillsBudget + totalConsumptionBudget);
        ...
        <div className="bg-[hsl(var(--bg-secondary))] p-3 border border-[hsl(var(--border-color))] rounded-lg shadow-xl">
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} style={{ color: entry.color }} className="text-sm font-medium">
              {entry.name === 'planlagt' ? 'Planlagt saldo' : 'Faktisk saldo'}: {entry.value.toLocaleString('da-DK')} kr.
            </p>
          ))}
        </div>
      );
  ...
  const getStatusText = (actual: number, planned: number, inverted: boolean = false) => {
    ...
    if (diff === 0) return "præcis som planlagt";
    const isMore = diff > 0;
    ...
    return (
      <span className={isMore ? (inverted ? "text-green-500" : "text-[hsl(var(--brand-danger))]") : (inverted ? "text-[hsl(var(--brand-danger))]" : "text-green-500")}>
        {isMore ? 'mere' : 'mindre'} end planlagt
      </span>
    );
  };
      ...
      <div className="flex-1 space-y-6">
        <div>
          <h2 className="text-2xl font-light text-[hsl(var(--text-primary))]">
            Resultat for mit budget i <span className="font-bold">{currentYear}</span>
          </h2>
          <p className="text-muted">
            Saldo i slutningen af {currentMonthName} var <span className="font-bold text-[hsl(var(--text-primary))]">{currentActualBalance.toLocaleString('da-DK')} kr.</span>
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card className="bg-[hsl(var(--bg-secondary))] border-none shadow-sm">
            <CardContent className="p-4 flex flex-col items-center justify-center text-center h-full">
              <span className="text-sm text-muted mb-2">Saldo ultimo {currentMonthName}</span>
          ...
          <Card className="border-none shadow-sm">
            <CardContent className="p-4 flex flex-col items-center justify-center text-center h-full">
              <span className="text-sm text-muted mb-2">Indkomst</span>
              <span className="text-xl font-bold mb-1">{totalIncome.toLocaleString('da-DK')} kr</span>
              <span className="text-xs font-medium">{getStatusText(totalIncome, totalIncomeBudget, true)}</span>
            </CardContent>
          </Card>
          <Card className="border-none shadow-sm">
            <CardContent className="p-4 flex flex-col items-center justify-center text-center h-full">
              <span className="text-sm text-muted mb-2">Regninger</span>
              <span className="text-xl font-bold mb-1">{totalBills.toLocaleString('da-DK')} kr</span>
              <span className="text-xs font-medium">{getStatusText(totalBills, totalBillsBudget)}</span>
            </CardContent>
          </Card>
          <Card className="border-none shadow-sm">
            <CardContent className="p-4 flex flex-col items-center justify-center text-center h-full">
              <span className="text-sm text-muted mb-2">Forbrug</span>
              <span className="text-xl font-bold mb-1">{totalConsumption.toLocaleString('da-DK')} kr</span>
              <span className="text-xs font-medium">{getStatusText(totalConsumption, totalConsumptionBudget)}</span>
            </CardContent>
          </Card>
        </div>

        <div className="pt-6">
          <h3 className="text-xl font-light mb-1">Budgetsaldo</h3>
          <p className="text-sm text-muted mb-6">Går det bedre eller dårligere end hvad jeg har budgetteret med?</p>
      ...
      <div className="xl:w-80 space-y-6">
        <Card className="bg-[hsl(var(--bg-secondary))] border-none text-center pt-6">
          <CardContent className="flex flex-col items-center">
            <h3 className="text-xl font-light mb-6">Mit budget {currentYear}</h3>
            ...
            <div className="flex flex-col items-center border-t border-[hsl(var(--border-color))] pt-6 w-full">
              <span className="text-xs font-bold text-muted uppercase tracking-wider mb-2">Slutsaldo {currentYear} (Prognose)</span>
              <span className="text-3xl font-bold">{projectedBalance.toLocaleString('da-DK')} kr.</span>
              <span className="text-xs text-muted mt-1">Budget {projectedBalance.toLocaleString('da-DK')}</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-[hsl(var(--bg-secondary))] border-none">
          <CardContent className="p-6">
            <h3 className="font-medium mb-4 text-lg">Budget gns/måned</h3>
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                <span className="text-muted">Indkomst</span>
                <span className="font-medium">{Math.round(avgIncome).toLocaleString('da-DK')} kr</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted">Regninger</span>
