...
export function BudgetDetailsSidebar({ category, year, onClose }: BudgetDetailsSidebarProps) {
  ...
  const { data: budgetBills, isLoading: isLoadingBills } = useBudgetBills(
    isFixedOrIncome ? category.category_id : null,
    year
  );
  ...
  const { data: monthTransactionsData, isLoading: isLoadingTx } = useTransactions(
    ...
    undefined,
    undefined,
    selectedMonth ? category.category_id : undefined
  );
  ...
  const handleSave = () => {
    if (isFixedOrIncome) {
      ...
      for (let month = 1; month <= 12; month++) {
        const amountMinor = activeMonths.has(month)
          ? Math.round(safeAmount * 100) * signMultiplier
          : 0;
        promises.push(
  ...
  const chartData = Array.from({ length: 12 }, (_, i) => {
    ...
    if (isFixedOrIncome) {
      ...
      explicitBudgetMinor = activeMonths.has(monthNum)
        ? Math.round(parsedAmount * 100) * signMult
        : 0;
    }
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 w-full md:w-[450px] bg-[hsl(var(--bg-secondary))] shadow-2xl z-[70] flex flex-col border-l border-[hsl(var(--border-color))]">
        <div className="flex items-center justify-between p-4 border-b border-[hsl(var(--border-color))]">
          <h2 className="text-lg font-semibold capitalize truncate pr-4">
            {label.replace('-', ' ')}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          <div className="space-y-4">
            <h3 className="text-sm font-medium text-muted">
              {t('budgets.yearly_overview', 'Årsoversigt')} {year}
            </h3>
            <div className="bg-[hsl(var(--bg-primary))] p-4 rounded-lg border border-[hsl(var(--border-color))] h-[250px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                  ...
                  <Bar
                    dataKey={t('budgets.budgeted', 'Budget')}
                  ...
                  <Bar
                    ...
                    onClick={(data, index) =>
                      setSelectedMonth(
                        selectedMonth !== null && index === selectedMonth - 1 ? null : index + 1
                      )
                    }
                    cursor="pointer"
                  >
            ...
            <div className="space-y-3 animate-fade-in border-t border-[hsl(var(--border-color))] pt-6">
              ...
              {isLoadingTx ? (
                ...
                <p className="text-sm text-muted text-center p-4">
                  Ingen transaktioner i denne måned
                </p>
              ) : (
                <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2">
                  {monthTransactions.map((tx: any) => (
                    <div
                      key={tx.id}
                      className="bg-[hsl(var(--bg-primary))] p-3 rounded-lg border border-[hsl(var(--border-color))] flex justify-between items-center"
                    >
                      <div className="flex flex-col min-w-0 pr-4">
                        <span className="text-sm font-medium truncate">{tx.description}</span>
                        <span className="text-xs text-muted">
                          {format(parseISO(tx.booking_date), 'dd. MMM', {
                            locale: i18n.language === 'en' ? enUS : da,
                          })}
                        </span>
                      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4 border-b border-[hsl(var(--border-color))] pb-4">
                {bills.map((bill, index) => (
                  <div
                    key={bill.id}
                    className="p-4 bg-[hsl(var(--bg-primary))] border border-[hsl(var(--border-color))] rounded-lg space-y-3"
                  >
              ...
              <div className="flex justify-between items-center text-sm font-medium">
                <span className="text-[hsl(var(--text-primary))]">
                  {t('budgets.yearly_total', 'Samlet (pr. år)')}
                </span>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-sm text-[hsl(var(--text-primary))]">
                    {t('budgets.rollover', 'Løbende budget')}
                  </p>
                  <p className="text-xs text-muted">
                    {t('budgets.rollover_desc', 'Overfør overskud/underskud til næste måned')}
                  </p>
                </div>
              </div>
        ...
        <div className="p-4 pb-28 md:pb-4 border-t border-[hsl(var(--border-color))] bg-[hsl(var(--bg-secondary))] flex justify-end gap-3">
          ...
          >
            <Save size={16} />
            {upsertMutation.isPending || saveBillsMutation.isPending
              ? t('common.loading', 'Gemmer...')
              : t('common.save', 'Gem')}
          </Button>
        </div>
      </div>
