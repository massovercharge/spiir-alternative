...
import {
  ...
  useSplitTransaction,
  useLinkReceiptToTransaction,
  useSuggestedReceipts,
  useDismissDuplicate,
} from '../../api/client';
...
export function TransactionDetailsSidebar({
  ...
  const { data: suggestions, isLoading: isLoadingSuggestions } = useSuggestedReceipts(
    transaction?.id,
    isLinkingReceipt
  );
  ...
  const handleSave = () => {
    ...
    if (isExtraordinary !== (transaction.is_extraordinary || false))
      patch.is_extraordinary = isExtraordinary;
    patch.tags = tags;
    ...
    if (isSplitMode) {
      ...
      updateMutation.mutate({ transactionIds: [transaction.id], patch }, { onSuccess: onClose });
    }
  };
  ...
  const addSplit = () => {
    ...
    const amountStr =
      Math.abs(missingAmount) > 0.01 ? missingAmount.toFixed(2).replace('.', ',') : '';
  ...
  const getSearchTerm = (desc: string) => {
    ...
    term = term.replace(/dankort[- ]?køb/gi, '');
    term = term.replace(/dankort[- ]?nota/gi, '');
    term = term.replace(/visa\/dankort/gi, '');
    term = term.replace(/\bkontaktløs\b/gi, '');
    term = term.replace(/\bvisa\b/gi, '');
    term = term.replace(/\bdankort\b/gi, '');
    term = term.replace(/\bmastercard\b/gi, '');
    term = term.replace(/\bmobilepay\b|\bmobilpay\b/gi, '');
    term = term.replace(/\bn\*\d+\b/gi, '');
    term = term.replace(/\bnet\d+\b/gi, '');

    // 3. Remove nota / notanr and associated receipt/transaction reference codes
    term = term.replace(
      /\b(?:dankort[- ]?|visa(?:[/-]dankort)?[- ]?)?nota(?:\s*nr\.?|\.nr\.?|\.|\:)?\s*[a-z0-9]+\b/gi,
      ''
    );
    term = term.replace(/\bnotanr\.?\s*[a-z0-9]+\b/gi, '');
    term = term.replace(/\b(?:dankort[- ]?|visa(?:[/-]dankort)?[- ]?)?nota\b/gi, '');
    term = term.replace(/\bnotanr\.?\b/gi, '');

    // 4. Remove common transaction filler words
    term = term.replace(/betaling til/gi, '');
    term = term.replace(/køb dkk/gi, '');
    term = term.replace(/køb/gi, '');
    ...
    term = term.replace(/(?<!aftalenr\s*)(?<!aftale\s*nr\s*)\b\d{4,}\b/gi, '');
    ...
    <>
      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-md bg-[hsl(var(--bg-primary))] border-l border-[hsl(var(--border-color))] shadow-2xl z-[70] overflow-y-auto flex flex-col animate-slide-in-right">
        <div className="p-6 border-b border-[hsl(var(--border-color))] flex justify-between items-start gap-4">
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-[hsl(var(--bg-secondary))] rounded-full shrink-0"
          >
        ...
        <div className="p-6 flex-1 space-y-6">
                  ...
                  onClick={() => {
                    ...
                    dismissDuplicateMutation.mutate(
                      { transaction_ids: ids },
                      {
                        onSuccess: () => {
                          toast.success(
                            t('duplicates.dismissSuccess', 'Advarsel skjult for denne postering')
                          );
                          onClose();
                        },
                      }
          ...
          {isSplitMode ? (
            <div className="space-y-4">
              ...
              <div
                className={`text-sm mt-4 p-3 rounded-lg flex items-center gap-2 ${isSumValid ? 'bg-green-500/10 text-green-600' : 'bg-yellow-500/10 text-yellow-600'}`}
              >
                {isSumValid ? <Check size={16} /> : <AlertCircle size={16} />}
                <div>
                  Fordelt:{' '}
                  <span className="font-semibold">{currentSum.toFixed(2).replace('.', ',')}</span>
                  {!isSumValid && (
                    <span> (Mangler {(targetSum - currentSum).toFixed(2).replace('.', ',')})</span>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="space-y-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                {customDate !== transaction.booking_date && (
                  <p className="text-xs text-muted">
                    Oprindelig bankdato:{' '}
                    {format(new Date(transaction.booking_date), 'd. MMM yyyy', {
                      locale: dateLocale,
                    })}
                  </p>
                )}
            ...
            {(transaction.booking_date_time ||
              ...
              <div className="pt-4 border-t border-[hsl(var(--border-color))] space-y-2 text-sm text-muted">
                <h3 className="font-semibold text-[hsl(var(--text-primary))] uppercase tracking-wider text-xs mb-3">
                  Bankdetaljer (PSD2)
                </h3>
                ...
                {transaction.booking_date_time && (
                  <div className="flex justify-between">
                    <span>Tidspunkt:</span>
                    <span>
                      {format(new Date(transaction.booking_date_time), 'HH:mm:ss', {
                        locale: dateLocale,
                      })}
                    </span>
                  </div>
                ...
                {transaction.balance_after_transaction_minor != null && (
                  <div className="flex justify-between">
                    ...
                    <span>
                      {(transaction.balance_after_transaction_minor / 100)
                        .toFixed(2)
                        .replace('.', ',')}{' '}
            ...
            <div className="pt-4 border-t border-[hsl(var(--border-color))]">
              {isSplitMode ? (
                <p className="text-xs text-center text-muted mt-2">
                  Transaktionen er splittet. Summen skal matche totalbeløbet.
                </p>
              ) : (
                <>
                  <Button
                  ...
                  <p className="text-xs text-center text-muted mt-2">
                    Del transaktionen op i flere kategorier
                  </p>
                </>
              )}
            </div>
            ...
            <div className="pt-4 border-t border-[hsl(var(--border-color))] space-y-3">
              {isLinkingReceipt ? (
                <div className="space-y-3 bg-[hsl(var(--bg-secondary))] p-3 rounded-lg border border-[hsl(var(--border-color))]">
                  <div className="flex items-center justify-between pb-1 border-b border-[hsl(var(--border-color))]">
                    <div className="flex items-center gap-1.5 font-semibold text-xs text-primary uppercase tracking-wider">
                      <Sparkles size={14} />
                      <span>
                        {t('transactions.suggestedReceipts', 'Matchende Storebox-kvitteringer')}
                      </span>
                    </div>
                    <Button
                      variant="ghost"
                  ...
                  {isLoadingSuggestions ? (
                    <p className="text-xs text-muted py-2 text-center">
                      {t('transactions.searchingSuggestions', 'Søger efter kvitteringer...')}
                    </p>
                        ...
                        >
                          <div className="flex items-center justify-between font-medium">
                            ...
                            <span className="text-[hsl(var(--text-primary))]">
                              {(sug.total_price_minor / 100).toFixed(2).replace('.', ',')}{' '}
                              {sug.currency}
                            </span>
                          </div>
                          ...
                          {sug.items_preview && sug.items_preview.length > 0 && (
                            <div className="text-muted text-[11px] truncate pt-1 border-t border-[hsl(var(--border-color))]/50">
                              {sug.items_preview
                                .map(
                                  (it) =>
                                    `${it.name} (${(it.amount_minor / 100).toFixed(2).replace('.', ',')} kr)`
                                )
                                .join(' • ')}
                            </div>
                          )}
                          ...
                          >
                            ...
                            {linkMutation.isPending
                              ? t('common.connecting', 'Forbinder...')
                              : t('transactions.linkThisReceipt', 'Forbind denne kvittering')}
                          </Button>
                        </div>
                      ))}
                    </div>
                    ...
                    <p className="text-xs text-muted py-2 text-center">
                      {t(
                        'transactions.noReceiptsFound',
                        'Ingen matchende kvitteringer fundet automatisk.'
                      )}
                    </p>
                  )}
                  ...
                  <div className="pt-2 border-t border-[hsl(var(--border-color))] space-y-2">
                    ...
                    >
                      <span>
                        {t(
                          'transactions.orEnterManualId',
                          'Eller indtast Storebox Receipt ID manuelt:'
                        )}
                      </span>
                    ...
                    {showManualReceiptInput && (
                      <div className="space-y-2 pt-1">
                        ...
                        <Button
                          ...
                          disabled={!receiptIdInput.trim() || linkMutation.isPending}
                        >
                  ...
                  {linkMutation.isError && (
                    <p className="text-xs text-red-500 mt-2">
                      {(linkMutation.error as Error).message || 'Der opstod en fejl'}
                    </p>
                  )}
                </div>
              ) : (
                <Button
                  variant="outline"
                  className="w-full flex items-center justify-center gap-2"
                  onClick={() => setIsLinkingReceipt(true)}
                >
                  <Plus size={16} />
